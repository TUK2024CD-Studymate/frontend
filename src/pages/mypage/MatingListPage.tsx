...
import { useApiUrlStore, useQuestionListStore, useLoadingStore } from '../../store/store.ts'
import SkeletonUI from '../../components/skeleton/SkeletonUI.tsx'

const Container = styled.div`
  display: flex;
  margin-top: 3rem;../../shared/components/skeleton/SkeletonUI.tsx
`
  ...
  const getQuestion = async (nickname: string) => {
    try {
      ...
      const MyQuestions = response.data.filter(
        (post: { writer: string }) => post.writer === nickname,
      ) //유저의 nickname 과 작성자의 wirter가 일치하는 질문
            ...
            questionList.map((list, index) => (
              <MyPost key={index}>
                <Content>
                  ...
                  <BoardType>
                    {list.interests === 'SERVER'
                      ? '서버/네트워크'
                      : list.interests === 'WEBAPP'
                      ? '웹/앱 개발'
                      : list.interests === 'DATA'
                      ? '데이터개발'
                      : list.interests === 'AI'
                      ? 'AI/IOT'
                      : list.interests === 'SECURITY'
                      ? '정보보안'
                      : null}
                  </BoardType>
                </Content>
                ...
                <FooterWrap>
                  <IsSolved>{list.isSolved ? '질문 해결 완료' : '질문 해결 미완료'}</IsSolved>
