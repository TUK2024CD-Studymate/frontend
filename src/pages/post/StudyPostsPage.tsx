...
const StudyPosts = styled(Link)<{ recruitmentStatus: boolean }>`
  ...
  color: ${({ recruitmentStatus }) =>
    recruitmentStatus ? '#000000' : '#e8e8e8'};
`;
...
const LikedIcon = styled(IoIosHeart)<{ recruitmentStatus: boolean }>`
  color: ${({ recruitmentStatus }) =>
    recruitmentStatus ? '#ff0000' : '#e8e8e8'};
  font-size: 1.5rem;
`;
...
const Detail = styled.div<{ recruitmentStatus: boolean }>`
  font-size: 1.75rem;
  color: ${({ recruitmentStatus }) =>
    recruitmentStatus ? '#9b9b9b' : '#e8e8e8'};
`;
  ...
  const OnFilter = (recruitmentStatus: boolean) => {
    if (isClicked && filterOption === recruitmentStatus.toString()) {
      ...
      const filterPost = postsList.filter(
        (post) => post.recruitmentStatus === recruitmentStatus,
      );
        ...
        .map((post) => (
          ...
          >
            ...
            <FooterWrapper>
              ...
              <Detail recruitmentStatus={post.recruitmentStatus}>
                {post.createdAt}
              </Detail>
              <Divider src={DividerImg} />
              <Detail recruitmentStatus={post.recruitmentStatus}>
              ...
              {Object.keys(recruitmentStatusLabels).map((status) => (
                <Btn
                  ...
                  active={
                    isClicked &&
                    filterList.some(
                      (post) => post.recruitmentStatus === (status === 'true'),
                    )
                  }
